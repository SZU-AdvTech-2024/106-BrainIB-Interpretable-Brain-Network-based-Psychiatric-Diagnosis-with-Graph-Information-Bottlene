...
def train(args, model, device, train_graphs, optimizer, epoch, SG_model):
    ...
    for pos in pbar:
        ...
        if pos % 10 == 0:
            writer.add_scalar('Training Loss', loss.item(), epoch * len(train_graphs) + pos)
...
for epoch in range(1, args.epochs + 1):
    ...
    writer.add_scalar('Training Accuracy', acc_train, epoch)
    writer.add_scalar('Test Accuracy', acc_test, epoch)
    writer.add_scalar('F1 Score', test_f1s, epoch)
    writer.add_scalar('MCC', test_mcc, epoch)
    scheduler.step()
writer.close()
